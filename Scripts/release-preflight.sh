...
# source line and fail forever on a repo with no leak in it at all. That
# includes THIS comment — every header below is spelled with the closing
# dashes deliberately left off, because writing them out would make the
# needle match its own explanation.
#
# The needle joins the words "PRIVATE KEY" to the closing "-----" rather
# than to the opening "BEGIN PRIVATE" (the pre-fix needle), because the
# closing dashes are the one substring common to all five headers
# OpenSSL/OpenSSH emit, not just the plain PKCS8 one:
#   -----BEGIN PRIVATE KEY            (plain PKCS8)
...
if [ -f fastlane/.env ]; then
    ...
    #
    # The remedy is NOT "run bootstrap": bootstrap calls asc_key, which does
    # ENV.fetch("ASC_KEY_ID") with no default, so it raises the moment .env
    # is absent — it cannot create the file this check is missing. .env is
    # written by hand; see docs/app-review/README.md's "Reconstruir
    # fastlane/ en una máquina nueva" section.
    fail "fastlane/.env is missing — cannot verify the issuer id has not leaked; it cannot be regenerated by bootstrap (bootstrap itself requires .env). Follow the rebuild steps in docs/app-review/README.md instead."
    cred_ok=0
fi
