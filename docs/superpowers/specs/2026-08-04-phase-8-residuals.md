...
## 12. Cierre de rama: la revisión final y su re-revisión

Antes de cerrar `feat/rejection-regression-automation`, una revisión de la rama entera —no limpia, según registra el §3 de este mismo documento— y una segunda pasada sobre esa misma revisión. Ningún comportamiento cambió; lo que sigue es prosa que decía de más, y lo que esa pasada dejó anotado sin tocar código.

### Corregido en esta pasada

- **"ninguno de los dos comprueba nada de eso" decía de más.** Tres sitios afirmaban, cada uno en su idioma, que `ColdStartTests` y `DatasetStoreTests` no comprueban ninguna de las invariantes cruzadas de `ShippedDatasetTests`: `CLAUDE.md` (~línea 203), el comentario del paso 3 en `Scripts/release-preflight.sh` (~línea 111) y la sección "Por qué" de `2026-08-04-rejection-regression-automation-design.md`. No es cierto: `DatasetStoreTests.listsResolve` sí afirma, con la misma fuerza, sobre la copia del bundle, la invariante que `ShippedDatasetTests.listsOnlyReferenceKnownEntries` afirma sobre el fichero en disco —que cada id de cada lista resuelve a una entrada conocida—, y `ColdStartTests.everyShelfIsRenderable` sí afirma una forma más débil de esa misma invariante —que cada lista resuelve a al menos una entrada, no a todas—, más una comprobación de copy sin equivalente en `ShippedDatasetTests`, porque `CuratedListCopy` vive en el lado de la app. Aun así, ninguna de las dos suites afirma el conjunto completo ni el barrido de secretos, ni abre el fichero directamente de disco. Las tres frases se reescribieron para decir eso; las afirmaciones que sí siguen siendo ciertas —que `ShippedDatasetTests` es la única que lee el fichero comprometido directamente de disco, la única que afirma el conjunto completo de invariantes cruzadas y el barrido de secretos, y la única que `xcodebuild test` nunca ejecuta— se mantuvieron.
- **Los docstrings del ancla `discoverTrendingMovies` afirmaban más de lo que prueba.** `Plotline/Support/AccessibilityAnchors.swift`, `PlotlineUITests/UITestAnchors.swift` y `PlotlineUITests/ColdStartUITests.swift` la describían como si su presencia probara que TMDB devolvió contenido. No es así: `MediaSection` (`Plotline/Views/Discovery/MediaSection.swift:60-73`, `placeholderView`) dibuja un shimmer cuando `items` está vacío, así que el contenedor anclado se renderiza igual con cero resultados que con cien. Lo que el ancla prueba es que **no se tomó la rama de error** —que el fetch no falló—, no que haya contenido: un 200 sin resultados la satisface igual. Los tres docstrings se reescribieron para decir eso; ni la aserción de `testDiscoverMatchesTheModeThisPassClaims` ni `DiscoveryView`/`MediaSection` se tocaron, y los dos ficheros de anclas siguen llevando los mismos literales, byte a byte.
- **"los seis tests" contaba de menos.** La entrada de este mismo documento sobre las dos pasadas que no comprobaban su modo (§3, "Y cuatro cosas corregidas después de escribir este documento") describía `ColdStartUITests` con seis métodos. El suite tiene siete desde que se añadió `testSettingsIsReachableAndPopulated`. Corregido a "los siete tests" en el mismo párrafo.

### Abierto, anotado en esta pasada

#### 12. La regla de `.gitignore` para `xcshareddata/` deja de ser cierta el día que haya un paquete

`.gitignore` ignora `Plotline.xcodeproj/project.xcworkspace/xcshareddata/` completo, con el comentario de que es "configuración de workspace por checkout, nada compartido que valga versionar". Esa carpeta también es donde Xcode fija las versiones de dependencias de Swift Package Manager, en `swiftpm/Package.resolved`. Hoy el proyecto no declara ni un `XCRemoteSwiftPackageReference` y `swiftpm/` está vacía salvo un subdirectorio `configuration` sin contenido, así que no se pierde nada. El comentario deja de ser cierto en cuanto alguien añada un paquete desde Xcode: `Package.resolved` fijaría versiones que dejarían de estar versionadas, y cada checkout resolvería las suyas.

#### 13. El log de `run_suite` existe y no se ve cuando más hace falta

`run_suite` ahora escribe la ejecución entera a `$SUITE_LOG` y solo enseña `tail -20` en pantalla; el paso 2 lee ese mismo fichero para confirmar el modo (§ "Ninguna de las dos pasadas…", arriba). Pero el `trap 'rm -f "$SUITE_LOG"' EXIT` lo borra al salir, pase lo que pase. Una pasada roja hoy deja exactamente el mismo material de diagnóstico que antes de que el log existiera: veinte líneas en pantalla y nada más, aunque el fichero completo existió durante toda la ejecución.

#### 14. El paso 1 no comprueba en el log que la pasada hambrienta corrió hambrienta

El paso 2 lee `$SUITE_LOG` para confirmar `PLOTLINE_UITEST_MODE_OBSERVED=live`; el paso 1 no tiene el equivalente para `starved`, y descansa solo en que `testDiscoverMatchesTheModeThisPassClaims` elija el ancla correcta según lo que `isLiveMode` observe dentro del proceso. En la práctica el hueco es teórico: `xcodebuild` solo reenvía al runner del simulador las variables con el prefijo `TEST_RUNNER_`, así que un `PLOTLINE_UITEST_MODE` suelto en el shell de quien lo ejecute no puede llegarle al proceso de test. El paso 1 no tiene guarda de log; no hay ninguna vía conocida de que la necesite.

### Confirmación del residuo nº 5

El residuo nº 5, arriba —el simulador se arranca una sola vez, fuera de `run_suite`, y la pasada viva es la que se queda sin correr si `xcodebuild` lo deja apagado— se releyó contra el script tal como queda después de esta pasada. Sigue exacto: sigue abierto, sigue sin arreglar, y la única vía de arreglo que apunta sigue siendo la misma.

### Lo más instructivo que dejó esta revisión

El brief que pidió el arreglo de las dos pasadas (§3, arriba) especificaba una guarda: si la pasada viva no corría, un `<SkippedTests>` en el esquema compartido debía sacarla del bucle por defecto de `xcodebuild test`. Esa guarda **habría sido ella misma infalsificable**. `-only-testing` no anula `<SkippedTests>` — la entrada seguía en el esquema, silenciosamente sin efecto sobre cómo se invoca el paso 2, y nada se lo habría dicho a nadie: ni un aviso, ni un rojo, ni una línea en el log. Se sustituyó por lo que el paso 2 usa hoy — cada pasada imprime en su propio log el modo que observó, y el script lo lee de vuelta y falla si no dice lo que ese paso afirma haber corrido. Una guarda contra guardas infalsificables que era ella misma infalsificable es, de todo lo que produjo esta revisión, lo que más vale la pena dejar escrito.
